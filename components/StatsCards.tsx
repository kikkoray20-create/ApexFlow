...
const StatsCards: React.FC<StatsCardsProps> = ({ orders, activeFilter, onStatusClick }) => {
  // Filter out Payment and Return entries so stats only show actual physical Orders
  const orderOnlyList = useMemo(() => {
    return orders.filter(o => o.status !== 'Payment' && o.status !== 'Return');
  }, [orders]);

  const getCount = (status: string) => orderOnlyList.filter(o => o.status.toLowerCase() === status.toLowerCase()).length;
  
  // Dashboard Total: All physical orders that aren't rejected
  const getDashboardTotal = () => orderOnlyList.filter(o => o.status !== 'rejected').length;
  
  // Pending: Physical orders with 'Pending' invoice status (excluding rejected)
  const getPendingInvoiceCount = () => orderOnlyList.filter(o => o.invoiceStatus === 'Pending' && o.status !== 'rejected').length;
